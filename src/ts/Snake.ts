...
class Snake
{
    ...
    constructor(_options: SnakeOptions) 
    {
    ...
    fatalCollisionDetection(_canvas)
    {
        ...
        if (this.body.length <= 4) return // no need to test self-collision if snake length < 4
    ...
    draw()
    {
        ...
        const gradient = context.createRadialGradient(
            centerOf(this.head.x, this.scale), // inner x
        ...
        this.context.arc(
            centerOf(this.head.x, this.scale), // x
    ...
    setDirectionTo(a: number, b: number)
    {
        this.direction.x = a
        this.direction.y = b
    }
